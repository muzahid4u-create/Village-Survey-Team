import { CUTOFF_DATE } from "./enums";
export function getSuggestedFamilyStatus(person) {
    if (person.relationToLandOwner === "PRIMARY_LAND_OWNER" ||
        person.relationToLandOwner === "PRIMARY_LAND_OWNER_SPOUSE") {
        return "PRIMARY";
    }
    if (person.relationToLandOwner === "DAUGHTER" &&
    ...
    const relationEligible = person.relationToLandOwner === "SON" || person.relationToLandOwner === "DAUGHTER";
    const marriedBeforeCutoff = person.maritalStatus === "MARRIED" &&
...
export function getFinalFamilyStatus(person) {
...
export function getDefaultFamilyGroupCode(status) {
    if (status === "PRIMARY" || status === "DEPENDENT") {
        return "F1";
    }
    return "";
}
export function isCompensationEligible(status) {
    return status !== "EXCLUDED";
}
export function buildPersonRecord(person) {
    const systemSuggestedStatus = getSuggestedFamilyStatus(person);
    const finalFamilyStatus = getFinalFamilyStatus(person);
    const familyGroupCode = finalFamilyStatus === "EXCLUDED"
        ? ""
...
export function buildFamilyGroups(persons) {
    ...
    for (const person of persons) {
        ...
        const code = person.finalFamilyStatus === "PRIMARY" || person.finalFamilyStatus === "DEPENDENT"
            ? "F1"
    ...
    return [...grouped.entries()]
        ...
        const head = members.find((member) => member.finalFamilyStatus === "PRIMARY") ??
